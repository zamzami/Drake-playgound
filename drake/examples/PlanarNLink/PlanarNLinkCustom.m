classdef PlanarNLinkCustom < PlanarRigidBodyManipulator
  
  methods
    function model = PlanarNLinkCustom(N) 
      ...
      for i=2:N-1
        model = addPlanarLink(model,2,2,.05);
      end
       model = addPlanarhand(model,0.1,0.1,.1);
  ...
  methods (Access=private)
    function model=addPlanarLink(model,mass,len,radius)
      ...
      function model=addPlanarpassiveLink(model,mass,len,radius)
      ...
      function model=addPlanarhand(model,mass,len,radius)
      ...
      limits.effort_max = 10;
      limits.velocity_limit = Inf;
      model = addJoint(model,['joint',num2str(ind-1)],'fixed',ind-1,ind,[0;0;-parentlen],zeros(3,1),model.view_axis,.1,[],[],[],limits);
      end
