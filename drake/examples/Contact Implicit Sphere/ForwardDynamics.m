...
f_base_position=[0;0;0.5];
f_base_orientation=rpy2quat(zeros(3,1));
configuration_vector=[0;0;0]; %wheel,arm1,arm2
dconfiguration_vector=zeros(9,1);
%run simulation
N=1000; tf=10;
step=tf/(N-1);
%ts = 0:0.01:12;
...
%x0 = [0;.3;zeros(6,1)];
%N=10; tf=.5;
robot = TimeSteppingRigidBodyManipulator(plant, 0.0001);
...
t = 0:step:tf;
wheel_torque_traj=PPTrajectory(foh(linspace(0,tf,N),0*linspace(0,tf,N)));
