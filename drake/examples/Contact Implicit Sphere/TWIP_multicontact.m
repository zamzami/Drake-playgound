...
%options.terrain = RigidBodyHeightMapTerrain([-5,1,1,2,2,5],[-1,1],0.5*[0 0 1 1 0 0;0 0 1 1 0 0]);
%options.terrain = RigidBodyFlatTerrain;
...
%warning(w);
 ...
 % Initial state  
f_base_position=[0;0.51];
f_base_orientation=zeros(1,1);
configuration_vector=[0;0;0]; %wheel,arm1,arm2
dconfiguration_vector=zeros(6,1);
x0 = [f_base_position;f_base_orientation;configuration_vector;dconfiguration_vector];
  
% Final state  
xf_f_base_position=[1.5;0.5];
xf_f_base_orientation=zeros(1,1);
xf_configuration_vector=[0;0;0]; %wheel,arm1,arm2
xf_dconfiguration_vector=zeros(6,1);
xf = [xf_f_base_position;xf_f_base_orientation;xf_configuration_vector;xf_dconfiguration_vector];

% mid state
x1_f_base_position=[0.5;1];
x1_f_base_orientation=zeros(1,1);
x1_configuration_vector=[0;-pi/4;pi/4]; %wheel,arm1,arm2
x1_dconfiguration_vector=randn(6,1);
...
% couple trajectory
  ...
  %traj_init.x = PPTrajectory(foh([0,tf],[x0,x0]));
   traj_init.l = PPTrajectory(foh(t_init,[repmat([C;C],1,Njump),repmat([C_mid;C_mid],1,N-Njump)]));
...
for i=1:length(scale_sequence)
 ...
 prog = prog.setSolverOptions('snopt','MajorFeasibilityTolerance',5e-12);
 prog = prog.setSolverOptions('snopt','MinorFeasibilityTolerance',1e-4);
 prog = prog.setSolverOptions('snopt','majoroptimalitytolerance',2e-6);
 prog = prog.setSolverOptions('snopt','linesearchtolerance',0.3);
 prog = prog.setSolverOptions('snopt','Violationlimit',100);
 prog = prog.setSolverOptions('snopt','print','PRINTAcu');
...
%   
  ...
  %prog = addStateConstraint(prog,ConstantConstraint(xf),N);
   ...
   prog = addStateConstraint(prog,ConstantConstraint(1.5),N,1); % xf
   ...
   % robot come to rest
    prog = addStateConstraint(prog,ConstantConstraint(zeros(6,1)),N,7:12);
%   
...
legendmatrixT{1}='T leg';
legendmatrixT{2}='T arm_1';
legendmatrixT{3}='T arm_2';
legendmatrixT{4}='T init leg';
legendmatrixT{5}='T init arm1';
legendmatrixT{6}='T init arm2';
...
%   
     figure (22);
  pu1=plot(ts,utraj_data(1:3,:),'.-', 'MarkerSize',25);
  title('$Torque$','interpreter','latex');
  xlabel('$t$','interpreter','latex','FontSize',30);
  ylabel('$torque$','interpreter','latex','FontSize',30);
  %drawnow;
  hold on 
  pu2=plot(ts,utraj_data_init(1:3,:),'-');
  legend(legendmatrixT)
  
       figure (23);
  pl1=plot(ts,ltraj_data(1:4,:),'.-', 'MarkerSize',25);
  title('$Lambda$','interpreter','latex');
  xlabel('$t$','interpreter','latex','FontSize',30);
  ylabel('$lambda$','interpreter','latex','FontSize',30);
  %drawnow;
  hold on 
  pl2=plot(ts,ltraj_data(5:8,:),'.-', 'MarkerSize',35);

  %pl3=plot(ts,ltraj_data_init(:,:),'-');
