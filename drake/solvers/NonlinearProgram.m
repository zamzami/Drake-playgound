...
%     iGfun,jGvar  % sparsity pattern in objective and nonlinear constraints
  ...
  methods
    function obj = NonlinearProgram(num_vars,x_name)
      ...
      obj.solver_options.snopt.Violationlimit = 10;
...
%         data{i} = obj.shared_data_functions{i}.eval(args{:});
    ...
    function obj = setSolverOptions(obj,solver,optionname,optionval)
      ...
      if(strcmpi(solver,'snopt'))
        if(strcmpi(optionname(~isspace(optionname)),'Violationlimit'))
          sizecheck(optionval,[1,1]);
          if(optionval<=0)
            error('Drake:NonlinearProgram:setSolverOptions:Violationlimit should be positive');
          end
          obj.solver_options.snopt.Violationlimit = optionval;
        elseif(strcmpi(optionname(~isspace(optionname)),'majorfeasibilitytolerance'))
          sizecheck(optionval,[1,1]);
          if(optionval<=0)
  ...
  methods(Access=protected)
    ...
    function [x,objval,exitflag,infeasible_constraint_name] = snopt(obj,x0)
      ...
      elseif(obj.which_snopt == 2)
        ...
        snsetr('Violation limit',obj.solver_options.snopt.Violationlimit);
